...
已有同码链的常用度重排由确定性会话流程处理。广告语 `重新排序 mkdr 编码链按常用度`、`把 mkdr 这条链按常用度排一下`、`重排 mkdr` 都会先读取实时编码链和当前草稿，再把 Create / Change / Delete 投影成一个有效合并视图。实时词映射成从类型基础权重开始递增的同词 `Change`，草稿词则直接更新当前草稿权重；不得因为实时链只有一条而忽略同码草稿词。程序必须把批次、版本、完整当前/建议状态、实时变更、草稿权重更新和计划摘要一起锁入票据，再展示一次共同确认；确认时只回放票据内的精确集合。任何集合变化、摘要变化、空链、字段不完整或证据无法形成安全计划都拒绝写入。即使无需调整，回复也必须逐条展示合并视图里的词、编码、权重和“词库 / 草稿”来源。

`重新排序下 米等、幂等-迷瞪`、以换行列词的同义形式，以及词表后置 `按优先级排`，必须先逐词解析实时词库加当前草稿投影。全部同码时仍走上述权重计划；各词的当前编码全部属于它们各自 encode 返回的同一候选前缀链时，将完整常用度顺序作为 `ordered_words` 交给 `keytao_shift_phrase_code`，由现有顺延 / 环形计划泛化为 N 项编码重分配，不得另写一套搬移逻辑。计划逐词展示当前与建议的编码和权重、证据、用户列出顺序的一致性，并只请求一次共同确认。
