...
- 首次调用（包括没有警告时）只返回完整预览，不写入；若当前本人已经用“加入”“加入并提交”、编号或明确编码授权了完全相同的目标，程序可在同一操作中自动回放精确预览票据，不再要求用户输入验证码
...
- 当前本人对 live 候选发送的明确选择可作为授权，程序按原样回放服务端预览参数；仅当预览扩大影响范围、包含其他草稿条目或涉及删除/顺延等高风险动作时，才额外询问一次。模型不得自行重新调用本工具或构造确认参数。
- 候选页显示为空位、但服务端预览时出现了新的 `warnings[]`，视为风险扩大；不得自动确认。此时只追加一次与 `batchId`、版本和风险摘要绑定的精确确认，旧确认不能命中新快照；不能只靠泛化的“确认加入”。
...
`keytao_submit_batch` 首次调用始终返回完整只读快照。程序会保存其中的 `batchId`、`contentVersion`、`snapshotDigest`、`warningDigest` 和 `auditDigest`。若用户当前指令已明确要求“提交”或“加入并提交”，且快照没有增加未授权条目，程序直接在同一操作中回放；否则只询问一次。模型不得自行构造 `confirmed=True` 或摘要字段。
...
机器人重启后，群聊用户若引用机器人本人发出且明确 @ 该用户的候选消息，并发送“加入并提交”，程序必须先用当前编码服务重新验证词语、候选编码和占用状态。验证通过后可以直接执行，但必须保守标记为需管理员审核；缺少可信归属、跨用户引用或候选已变化时一律拒绝写入。
