...
**⚠️ Change 操作强制流程：先查后改**

Change 操作的 `old_word` 必须来自词库的真实数据，不能凭用户输入或推测填写。

**Step 1：先调 `keytao_lookup_by_codes_batch` 查出所有目标编码的当前词**
```python
keytao_lookup_by_codes_batch(["fpnm", "jcnm", ...])  # 所有要修改的编码
```

**Step 2：用查询结果的 `word` 作为 `old_word`，用户期望的新词作为 `word`，再调 `keytao_batch_add_to_draft`**
```python
keytao_batch_add_to_draft(items=[
  {"action": "Change", "old_word": "防粘", "word": "防黏", "code": "fpnm"},  # old_word 来自 lookup 结果
  {"action": "Change", "old_word": "胶粘", "word": "胶黏", "code": "jcnm"},
])
```

若某个编码查不到词条，说明该词不存在，跳过该条目并告知用户。
