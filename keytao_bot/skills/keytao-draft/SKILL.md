...
{
  "success": true,
  "message": "成功写入 27 条，冲突 2 条，重码警告 1 条",
  "batchId": "uuid",
  "successCount": 27,
  "failedCount": 2,
  "skippedCount": 0,
  ...
  "warned": [
    {"index": 5, "word": "赠费", "code": "zrfwa", "reason": "编码 \"zrfwa\" 已被词条 \"增肥\" 占用，将创建重码（建议权重: 100）"}
  ],
  "draftItems": [...],
  "draftTotal": 27
}
...
成功 27 条，冲突 2 条，跳过 0 条，重码 1 条。

❌ 冲突条目（未写入）：
• 京豆 jgddo — 词条+编码已存在

⚠️ 重码警告（已自动写入）：
• 赠费 zrfwa — 编码 "zrfwa" 已被词条 "增肥" 占用，将创建重码
...
格式规则：
- `warnedCount > 0` 时，在冲突条目后增加「⚠️ 重码警告（已自动写入）」段落
- 重码警告条目**已成功写入草稿**，告知用户无需再次添加
- `warnedCount == 0` 时省略该段落
...
**`keytao_create_phrase` / `keytao_batch_add_to_draft` 返回 `requiresConfirmation: true` 时：**
- 告知用户重码情况，询问是否继续
- 用户确认后，**立即用相同参数 + `confirmed=True` 重新调用 `keytao_create_phrase`**
