...
已有同码链的常用度重排由确定性会话流程处理。广告语 `重新排序 mkdr 编码链按常用度`、`把 mkdr 这条链按常用度排一下`、`重排 mkdr` 都会先按用户文本中的编码查询服务端完整链，再把证据支持的建议顺序映射为从类型基础权重开始递增的同词 `Change`。程序必须先取得 `batchId`、`contentVersion`、`warningDigest` 并把完整词条集合、建议权重和计划摘要一起锁入票据，再展示一次共同确认；确认时只回放票据内的精确集合。任何集合变化、摘要变化、空链、字段不完整或证据无法唯一排序都拒绝写入。
