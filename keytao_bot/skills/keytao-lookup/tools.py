...
TOOLS = [
    {
        ...
        "function": {
            ...
            "description": "只读查询1至12个词的本地语料频次、词典收录和常用度比较。BCC 现代频次仅取多领域/新闻/文学/口语最高每百万频次，jieba 作为回退或参考。古代汉语/近代汉语另列历史补充，仅双方现代四频道未收录且词典与 jieba 无明确方向时破平，按返回的判定及说明展示。并列保留，无数据逐词标明，不把缺失当零。不调用模型或网络，不修改词库或草稿。",
            "parameters": {
                "type": "object",
                "properties": {
                    "words": {
