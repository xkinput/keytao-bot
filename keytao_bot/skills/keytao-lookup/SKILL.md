...
按工具返回的次序展示「排名、词语、语料频次、词典收录、结论、历史补充」六列。保留并列与证据接近的结论；没有数据的词放在末尾标为「无数据」，不得把缺失改成零或因一个词缺失而拒绝整组查询。语料频次只是本地参考语料中的统计，不代表全网实时使用量。
...
执行器若给出已核验的调序或放置命令，只逐字转述该条；没有命令时只展示查询结果，不自行构造操作建议。用户明确要求调整编码链时才进入下面的修改流程。
