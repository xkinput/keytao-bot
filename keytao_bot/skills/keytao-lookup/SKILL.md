...
用户可直接发送 `重新排序 mkdr 编码链按常用度`、`把 mkdr 这条链按常用度排一下` 或 `重排 mkdr`。这类指令只把用户当前消息里的编码作为链级目标；词、类型和当前权重必须全部来自 `keytao_lookup_by_code` 的服务端记录，引用消息只是上下文，不能提供执行目标。

程序使用统一常用度比较器评估整条 `(code, type)` 链，包括“现代常用词相对仅有古语词典信号的词优先”规则。能够形成唯一顺序时，必须完整展示“当前 → 建议”、每个词的权重移动和逐词证据，并在服务端锁定完整 Change 集合后只请求一次共同确认；确认前不写草稿。编码不存在、空链、缺失服务端字段、无证据平局或证据冲突时明确询问，不生成修改；当前顺序已有证据支持时直接说明无需调整。
