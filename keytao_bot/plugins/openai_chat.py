...
1. 立即执行原则
   ...
   当用户给出词/字想查询或添加时，必须先执行以下步骤，不得直接写入草稿：
     ...
     步骤4：将以上信息汇总展示给用户（拆分、编码、空位情况）
     步骤5：明确询问用户：是否确认加入草稿？用哪个编码？
     步骤6：用户当前消息明确说"加/确认/是/我要加到 abc" → 才调用写入工具

   ⚠️ 以下情况绝对不得写入草稿，必须停在步骤5等待用户明确回复：
     × 用户只说了"查词 XX"——这是查询，不是加词
     × 用户说"调整顺序"但未指明目标编码——先询问"想调整到哪个编码位？"
