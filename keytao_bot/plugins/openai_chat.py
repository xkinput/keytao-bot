...
1. 立即执行原则
   ...
   【阶段一：查询并展示（只读，绝不写草稿）】
   触发：用户查词或提到想加某词
   操作：keytao_encode + keytao_lookup_by_word，再 keytao_lookup_by_codes_batch 查所有候选编码
   展示后，确认句必须把推荐编码（第一个空位）直接写入句子，格式：
     ...
     是否以编码 pztyo 将「跑通」加入草稿？也可回复编号选其他编码。

   ⚠️ 确认句必须明确写出推荐编码（"以编码 pztyo"），这样下一轮才能直接用。

   【阶段二：用户回复后直接写入，禁止再查询】
   ⚠️ 收到用户确认后，禁止调用任何查询工具（keytao_encode / keytao_lookup_*）！
...
async def get_openai_response(
    ...
    try:
        ...
        messages.append({
            ...
            "content": (
                "━━━ 当前请求边界 ━━━\n"
                "以上是历史记录（用于理解上下文，包括上一轮的查词结果和推荐编码）。\n"
                "以下是用户刚发出的新消息，这是本轮唯一需要处理的请求。\n"
                "⚠️ 禁止主动发起新操作（如主动加词、主动调整顺序），除非当前消息有明确指令。\n"
                "✅ 允许：若当前消息是确认（如'是/好/确定'），可依据历史中上一轮的推荐编码完成写入。"
            )
        })
