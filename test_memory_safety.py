...
class PendingIsolationTests(unittest.TestCase):
    ...
    def test_trusted_word_record_is_digest_bound_and_cas_replaced_only_after_claim(self) -> None:
        trusted_record = create_pending_trusted_word_record(
            "箭头",
            "jmtdu",
            "Phrase",
        )
        self.assertTrue(self.store.set(self.group_a, trusted_record))
        record = self.store.get_record(self.group_a)
        self.assertFalse(record.requires_reconfirmation)
        self.assertFalse(record.confirmation_armed)

        tampered = PendingTrustedWordRecord(
            word=trusted_record.word,
            code="jmtd",
            phrase_type=trusted_record.phrase_type,
            record_digest=trusted_record.record_digest,
        )
        self.assertFalse(trusted_word_record_is_complete(tampered))
        self.assertFalse(self.store.set(self.group_b, tampered))

        ticket = PendingToolConfirm(
            "keytao_batch_add_to_draft",
            {"items": [{
                "action": "Delete",
                "word": trusted_record.word,
                "code": trusted_record.code,
                "type": trusted_record.phrase_type,
            }]},
        )
        self.assertFalse(
            self.store.replace_claimed_trusted_word_record(record, ticket)
        )
        self.assertTrue(self.store.begin_execution(record))
        self.assertTrue(
            self.store.replace_claimed_trusted_word_record(record, ticket)
        )
        self.assertFalse(
        ...
        self.assertIsInstance(self.store.get(self.group_a), PendingToolConfirm)
...
class AdvertisedWordSetSelectionTests(unittest.TestCase):
    ...
    def test_natural_assent_resolves_one_live_set_and_asks_for_two(self) -> None:
        ...
        selected = self.resolve("加入草稿，然后就提交。")

        self.assertTrue(selected.matched)
        self.assertEqual(selected.snapshot_token, token)
        self.assertEqual(selected.resolved_words, self.WORDS)
        self.assertEqual(selected.ask, "")

        second_words = ("电子榨菜", "情绪价值", "班味")
        self.seed(second_words)
        ambiguous = self.resolve("加入草稿，然后就提交。")

        self.assertTrue(ambiguous.matched)
        self.assertEqual(ambiguous.resolved_words, ())
        self.assertIn("两组", ambiguous.ask)
        self.assertIn("显眼包", ambiguous.ask)
        self.assertIn("电子榨菜", ambiguous.ask)
        self.assertEqual(len(self.store.advertised_word_sets(self.owner)), 2)
...
class PendingPersistenceTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_trusted_word_record_survives_restart_without_becoming_a_write_ticket(self) -> None:
        store = SQLiteConversationStateStore(self.db_path, clock=self.clock)
        trusted_record = create_pending_trusted_word_record(
            "箭头",
            "jmtdu",
        ...
        self.assertTrue(store.set(self.owner, trusted_record, owner_label="Owner"))
        original = store.get_record(self.owner)
        self.assertFalse(original.requires_reconfirmation)
        self.assertFalse(original.confirmation_armed)

        restored = SQLiteConversationStateStore(self.db_path, clock=self.clock)
        record = restored.get_record(self.owner)

        self.assertIsInstance(record.state, PendingTrustedWordRecord)
        self.assertTrue(trusted_word_record_is_complete(record.state))
        self.assertEqual(record.state, trusted_record)
...
class PlatformNeutralPendingTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_natural_single_candidate_assent_is_state_derived_and_fail_closed(
        ...
        conv_key = ConversationAddress.group(
            "qq",
            "natural-assent-group",
            "3755240737",
        )
        space_key = conv_key.space_key

        def seed_state() -> PendingAddWord:
            state = PendingAddWord(
                word="达致",
                recommended_code="dsfkv",
                candidates=[("dsfk", True), ("dsfkv", False)],
                occupied_words={"dsfk": ["大致"]},
                server_candidates=[("dsfk", True), ("dsfkv", False)],
                server_occupied_words={"dsfk": ["大致"]},
            )
            state_store.set(
                conv_key,
                state,
        ...
        classifier = AsyncMock(return_value=chat_module.MessageCommandIntent())
        add_only = AsyncMock(return_value="✅ 已加入草稿")
        add_submit = AsyncMock(return_value="✅ 已加入草稿并提交审核")
        try:
            with (
                patch.object(
                    chat_module,
                ...
                patch.object(chat_module, "_execute_add_to_draft", add_only),
                patch.object(
                    chat_module,
                ...
                for message, submit_after in accepted.items():
                    with self.subTest(message=message):
                        seed_state()
                        response = await chat_module.handle_pending_message_core(
                            message,
                        ...
                        self.assertIn("已加入草稿", response)
                        called = add_submit if submit_after else add_only
                        called.assert_awaited_once()
                        self.assertEqual(called.await_args.args[:2], ("达致", "dsfkv"))
                        add_only.reset_mock()
                ...
                for message, marker in rejected.items():
                    with self.subTest(message=message):
                        seed_state()
                        response = await chat_module.handle_pending_message_core(
                            message,
    ...
    async def test_every_single_candidate_shape_advertises_executable_forms(
        self,
    ) -> None:
        from keytao_bot.plugins import openai_chat as chat_module
        ...
        state_store = MemoryConversationStateStore()
        chat_module.conversation_state_store = state_store
        correction_state = PendingAddWord(
            word="达致",
            recommended_code="dsfkv",
            candidates=[("dsfk", True), ("dsfkv", False)],
            server_candidates=[("dsfk", True), ("dsfkv", False)],
        ...
        try:
            with patch.object(
                chat_module,
                "call_tool_function",
                AsyncMock(return_value=json.dumps(correction_encoding, ensure_ascii=False)),
            ):
                corrected = await chat_module._try_update_pending_pronunciation(
                    correction_state,
                    "达读da",
                    "qq",
                    "candidate-shape-correction",
                    None,
                    "阿邦",
                )

            rendered_shapes = {
                "tool-encoded": encoded,
                "reviewed": reviewed,
                "pronunciation-corrected": corrected,
            }
            add_only = AsyncMock(return_value="✅ 已加入草稿")
            add_submit = AsyncMock(return_value="✅ 已加入草稿并提交审核")
            batch_add = AsyncMock(return_value="✅ 已按选择加入草稿")
            with (
                patch.object(chat_module, "_execute_add_to_draft", add_only),
                patch.object(
                    chat_module,
                ...
                patch.object(chat_module, "_execute_confirmed_tool", batch_add),
            ):
                for shape, rendered in rendered_shapes.items():
                    with self.subTest(shape=shape):
                        self.assertIsNotNone(rendered)
                        contract = chat_module.advertised_reply_contract(rendered)
                        self.assertEqual(
                            contract.batch_assent_forms,
                            ("加入", "加入并提交"),
                        )
                        self.assertTrue(contract.candidate_selection)
                        self.assertNotIn("「都加」", rendered)
                        state = chat_module._parse_pending_add_word(rendered)
                        self.assertIsInstance(state, PendingAddWord)
                        state.server_candidates = list(state.candidates)
                        state.server_occupied_words = dict(state.occupied_words)
                        conv_key = ConversationAddress.private(
                            "qq",
                            f"candidate-shape-{shape}",
                        )

                        for form, submit_after in (
                            ("加入", False),
                            ("加入并提交", True),
                        ):
                            state_store.set(conv_key, state, owner_label="阿邦")
                            response = await chat_module.handle_pending_message_core(
                                form,
                                "qq",
                                conv_key.actor_id,
                        ...
                        selection_command = selection_match.group(1)
                        self.assertIsNotNone(
                            chat_module.parse_pending_candidate_selection(
                                selection_command,
                            )
                        )
                        state_store.set(conv_key, state, owner_label="阿邦")
                        selected = await chat_module.handle_pending_message_core(
                            selection_command,
                            "qq",
                            conv_key.actor_id,
                            conv_key,
                        ...
                        self.assertIn("已按选择加入草稿", selected)
                        batch_add.assert_awaited_once()
                        selected_state = batch_add.await_args.args[0]
                        self.assertEqual(
                            [
                                (item["word"], item["code"])
                                for item in selected_state.args["items"]
    ...
    async def test_numbered_add_submit_and_unverified_remediation_end_to_end(self) -> None:
        from keytao_bot.plugins import openai_chat as chat_module
        ...
        async def exercise(message: str, *, request_suggestion: bool = False):
            ...
            try:
                ...
                reply = await chat_module.handle_pending_message_core(
                    ...
                    space_key=conv_key.space_key,
                    owner_label="S15",
                    allow_intent_model=not request_suggestion,
                )
        ...
        self.assertIsNone(quoted_reply)
...
class MutationAuthorizationTests(unittest.TestCase):
    ...
    def test_dictionary_delete_and_two_move_plan_are_exact_whole_messages(self) -> None:
        delete_forms = {
            "删词 五百万": ("五百万", ""),
            "删词 五百万 wbw": ("五百万", "wbw"),
            "删除词库里的「五百万」": ("五百万", ""),
            "把 jmtdu 的箭头删掉": ("箭头", "jmtdu"),
            "删除 箭头 @ jmtdu": ("箭头", "jmtdu"),
        }
        for message, expected in delete_forms.items():
            with self.subTest(message=message):
                command = parse_dictionary_delete_command(message)
                self.assertEqual(
                    (command.word, command.code) if command else None,
                    expected,
                )
                self.assertTrue(message_authorizes_mutation(message))

        move_plan = parse_entry_move_plan(
            "把「箭头」换到jmtd，把「剪贴」换到jmtdoa"
        )
        self.assertEqual(
            tuple((move.word, move.target_code) for move in move_plan.moves)
            if move_plan else None,
            (("箭头", "jmtd"), ("剪贴", "jmtdoa")),
        )
        self.assertTrue(message_authorizes_mutation(
            "把「箭头」换到jmtd，把「剪贴」换到jmtdoa"
        ...
        for message in (
            ...
            "他说把 jmtdu 的箭头删掉",
            "删除 箭头 @ jmtdu？",
            "如果把「箭头」换到jmtd，把「剪贴」换到jmtdoa会怎样？",
        ):
            with self.subTest(message=message):
                self.assertIsNone(parse_dictionary_delete_command(message))
                self.assertIsNone(parse_entry_move_plan(message))
                self.assertFalse(message_authorizes_mutation(message))
    ...
    def test_every_front_insert_recommendation_line_round_trips(self) -> None:
        rendered = front_insert_recommendation_copy({
            "newWord": "洞洞鞋",
            "occupantWord": "多段线",
            "occupantCode": "ddx",
            "freeCode": "ddxu",
        ...
        recommendation_lines = [
            ...
            if "占 ddx" in line or "顺延" in line
        ]
        self.assertEqual(len(recommendation_lines), 1)
        for line in recommendation_lines:
            with self.subTest(line=line):
                self.assertTrue(message_authorizes_mutation(line))
    ...
    def test_eviction_modified_add_is_one_closed_operation(self) -> None:
        ...
        recommendation = "「洞洞鞋」占 ddx、「多段线」顺延"
        parsed = parse_eviction_modified_add(recommendation)
        self.assertIsNotNone(parsed)
        self.assertEqual(
            (
                parsed.word,
        ...
        self.assertTrue(message_authorizes_mutation(recommendation))
        self.assertTrue(message_authorizes_mutation(
            render_executable_suggestion(
                recommendation,
                words=("洞洞鞋", "多段线"),
...
class AlreadyExistingRenderingTests(unittest.TestCase):
    def test_already_existing_word_leads_single_multi_and_sink_copy(self) -> None:
        ...
        status = {
            ...
            "label": "已有「参考价值」",
            "words": ["参考价值"],
            "phrases": [{"word": "参考价值", "code": "ckjf"}],
        }
        reviewed = _format_reviewed_add_prompt({
            "success": True,
            "word": "参考价值",
            "recommendedCode": "ckjf",
            "existing": [{"word": "参考价值", "code": "ckjf"}],
            "pronunciations": [{
                "pinyin": "can kao jia zhi",
                "recommendedCode": "ckjf",
                "sources": [],
                "candidateStatuses": [
                    status,
                    {"code": "ckjfu", "occupied": False, "label": "空位"},
                ],
            }],
        }) or ""
        self.assertTrue(reviewed.startswith("「参考价值」已在词库（ckjf）"))
        self.assertNotIn("词库暂无收录「参考价值」", reviewed)
        self.assertIn("仍可选择其他编码", reviewed)

        single = render_server_backed_single_word_candidates(
            "参考价值",
            "ckjf",
            (("ckjf", True), ("ckjfu", False)),
            {"ckjf": ("参考价值",)},
        )
        self.assertTrue(single.startswith("「参考价值」已在词库（ckjf）"))
        ...
        scopes = [
            {
                "word": "参考价值",
                "candidates": [["ckjf", True], ["ckjfu", False]],
                "occupiedWords": {"ckjf": ["参考价值"]},
            },
            {
                "word": "情绪价值",
                "candidates": [["qxjf", True], ["qxjfu", False]],
                "occupiedWords": {"qxjf": ["情绪价值"]},
            },
        ]
        multi = render_server_backed_batch_candidates(items, scopes)
        self.assertIn("1. 「参考价值」\n   「参考价值」已在词库（ckjf）", multi)
        self.assertIn("2. 「情绪价值」\n   「情绪价值」已在词库（qxjf）", multi)
        lookup = render_server_backed_batch_lookup(items, scopes)
        self.assertNotIn("以上推荐仅用于本次查询", lookup)

        sink = plain_warning_message({
            "warningType": "duplicate_code",
            "item": {"word": "参考价值", "code": "ckjf"},
            "existing": [{"word": "参考价值", "code": "ckjf"}],
        })
        self.assertTrue(sink.startswith("「参考价值」已在词库（ckjf）"))


class RecentOwnDraftSubmitTests(unittest.IsolatedAsyncioTestCase):
    async def test_delivered_write_receipt_stages_only_its_actor_batch(self) -> None:
        ...
        context = ChatMemoryContext(
            ...
            space_type="group",
            space_id="receipt-group",
            speaker_name="Ealin",
        )
        old_store = commands.conversation_state_store
        commands.conversation_state_store = state_store
        deliveries_token = commands.current_draft_delivery_claims.set([])
        memory_token = commands.current_memory_context.set(context)
        try:
            commands._capture_successful_draft_write_delivery(
                "keytao_shift_phrase_code",
                {"success": True, "batchId": "batch-receipt"},
                "qq",
                "receipt-actor",
            )
            ...
            commands.current_draft_delivery_claims.reset(deliveries_token)
            commands.conversation_state_store = old_store

        record = state_store.get_record(context.conversation_address)
        self.assertIsNotNone(record)
        self.assertEqual(record.owner_key.actor_id, "receipt-actor")
        self.assertEqual(record.state.args["_recent_batch_ids"], ["batch-receipt"])

    async def test_confirm_and_submit_consumes_one_recent_own_batch(self) -> None:
        from keytao_bot.plugins import openai_chat as chat_module
        from keytao_bot.plugins.chat_commands import DraftActionResult

        state_store = MemoryConversationStateStore()
        conv_key = ConversationAddress.group("qq", "group-40", "actor-40")
        recent = PendingToolConfirm(
            function_name="keytao_submit_batch",
            args={
                "batch_id": "batch-own",
                "_recent_own_write": True,
                "_recent_batch_ids": ["batch-own"],
            },
            confirmation_source="local_preview",
        )
        self.assertTrue(state_store.set(conv_key, recent, owner_label="Ealin"))
        ctx = chat_module.TurnContext(
            bot=object(),
            event=object(),
            platform="qq",
            user_id="actor-40",
            normalized_message_text="确认并提交",
            memory_context=ChatMemoryContext(
                platform="qq",
                user_id="actor-40",
                space_type="group",
        ...
        submit = AsyncMock(return_value=DraftActionResult(
            "✅ 草稿已提交审核。",
            success=True,
            data={"batchId": "batch-own"},
        ))
        old_store = chat_module.conversation_state_store
        chat_module.conversation_state_store = state_store
        try:
            await chat_module._stage_resolve_current_pending_scope(ctx)
            self.assertEqual(
                getattr(ctx.scoped_pending_intent, "intent", ""),
                "pending_confirm",
            )
            with patch.object(
                chat_module,
                ...
                await chat_module._stage_execute_pending_state(ctx)
        finally:
            chat_module.conversation_state_store = old_store

        self.assertEqual(ctx.response, "✅ 草稿已提交审核。")
        self.assertIsNone(state_store.get_record(conv_key))
        self.assertEqual(submit.await_count, 1)
        self.assertEqual(submit.await_args.kwargs["batch_id"], "batch-own")
        self.assertTrue(submit.await_args.kwargs["auto_confirm"])

    async def test_recent_write_with_multiple_batches_refuses_truthfully(self) -> None:
        from keytao_bot.plugins import openai_chat as chat_module

        state_store = MemoryConversationStateStore()
        conv_key = ConversationAddress.private("qq", "actor-many")
        state_store.set(
            conv_key,
            PendingToolConfirm(
                function_name="keytao_submit_batch",
                args={
                    "batch_id": "",
                    "_recent_own_write": True,
                    "_recent_batch_ids": ["batch-a", "batch-b"],
                },
            ),
        )
        ctx = chat_module.TurnContext(
        ...
        try:
            ...
            chat_module.conversation_state_store = old_store
        self.assertIn("多个草稿批次", ctx.scoped_pending_response or "")
        self.assertIsNone(state_store.get_record(conv_key))
...
class EntrySwapHandlerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_literal_pair_resolves_both_words_before_ring_shift(self) -> None:
        ...
        async def fake_call(name, arguments, *_args, **_kwargs):
            if name == "keytao_lookup_by_words_batch":
                self.assertEqual(arguments, {"words": ["冒菜", "茂才"]})
                return json.dumps({
                    "success": True,
                    "results": [
                        {"word": "冒菜", "phrases": [{
            ...
            self.assertEqual(name, "keytao_list_draft_items")
            return json.dumps({
                "success": True,
                "batchId": "",
                "contentVersion": 0,
    ...
    async def test_ambiguous_word_never_reaches_ring_shift(self) -> None:
        ...
        async def fake_call(_name, arguments, *_args, **_kwargs):
            if _name == "keytao_list_draft_items":
                return json.dumps({
                    "success": True,
                    "batchId": "",
                    "contentVersion": 0,
            ...
            return json.dumps({
                ...
                "results": [
                    {"word": "冒菜", "phrases": [
                        {"word": "冒菜", "code": code,
                         "type": "Phrase", "weight": 100 + index}
                        for index, code in enumerate(("mzchi", "mzchii"))
                    ]},
                    {"word": "茂才", "phrases": [{
                        "word": "茂才", "code": "mzch",
                        "type": "Phrase", "weight": 100,
    ...
    async def test_same_code_priority_swap_uses_merged_rows_and_exact_weights(self) -> None:
        from keytao_bot.plugins import chat_commands
        from keytao_bot.plugins.chat_routing import MessageCommandIntent
        ...
        shift = AsyncMock(return_value="priority swap plan")
        with (
            patch.object(
                chat_commands,
                "_load_merged_word_entries",
        ...
        self.assertEqual(result, "priority swap plan")
        load.assert_awaited_once_with(("火锅", "电脑"), "qq", "priority-owner")
        shift.assert_awaited_once_with(
            "火锅",
            "mkdr",
            "qq",
...
class DictionaryDeleteAndMovePlanHandlerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_live_dictionary_delete_stages_one_exact_delete_ticket(self) -> None:
        ...
        execute = AsyncMock(return_value="locked delete confirmation")
        with (
            patch.object(
                chat_commands,
                "call_tool_function",
            ...
            result = await chat_commands._try_handle_dictionary_delete_command(
                "删词 五百万",
                MessageCommandIntent(
                    intent="dictionary_delete",
                    confidence=1.0,
        ...
        self.assertEqual(result, "locked delete confirmation")
        lookup.assert_awaited_once_with(
            "keytao_lookup_by_word",
            {"word": "五百万"},
            "qq",
        ...
        state = execute.await_args.args[0]
        self.assertIsInstance(state, PendingToolConfirm)
        self.assertEqual(state.function_name, "keytao_batch_add_to_draft")
        self.assertEqual(state.confirmation_source, "local_preview")
        self.assertEqual(state.args["items"], [{
            "action": "Delete",
            "word": "五百万",
            "code": "wbw",
    ...
    async def test_two_literal_moves_stage_one_exact_ring_plan(self) -> None:
        from keytao_bot.plugins import chat_commands
        from keytao_bot.plugins.chat_routing import MessageCommandIntent

        merged = {
            "箭头": {
                "word": "箭头", "code": "jmtdu", "type": "Phrase",
                "weight": 100, "source": "live",
            },
            "剪贴": {
                "word": "剪贴", "code": "jmtd", "type": "Phrase",
                "weight": 100, "source": "live",
            },
        }

        async def encode(_name, arguments, *_args, **_kwargs):
            self.assertEqual(_name, "keytao_encode")
            candidates = (
                ["jmtd", "jmtdu"]
                if arguments["word"] == "箭头"
                else ["jmtd", "jmtdoa"]
            )
            return json.dumps({
                "success": True,
        ...
        shift = AsyncMock(return_value="one move plan")
        with (
            patch.object(
                chat_commands,
                "_load_merged_word_entries",
            ...
            patch.object(chat_commands, "call_tool_function", side_effect=encode),
            patch.object(chat_commands, "_execute_shift_to_code", shift),
        ):
            result = await chat_commands._try_handle_entry_move_plan_command(
                "把「箭头」换到jmtd，把「剪贴」换到jmtdoa",
                MessageCommandIntent(
                    intent="entry_move_plan",
                    confidence=1.0,
                    keep_words=("箭头", "剪贴"),
                    requested_codes=("jmtd", "jmtdoa"),
                ),
                "qq",
                "move-owner",
                None,
        ...
        self.assertEqual(result, "one move plan")
...
class PendingCombinedIntentTests(unittest.TestCase):
    ...
    def test_bare_named_action_resolves_only_live_previous_turn_records(self) -> None:
        trusted_record = create_pending_trusted_word_record(
            "箭头",
            "jmtdu",
            "Phrase",
        )
        self.assertIsInstance(trusted_record, PendingTrustedWordRecord)
        self.assertTrue(trusted_word_record_is_complete(trusted_record))
        for action_name in ("换码", "删除", "删掉"):
            self.assertTrue(
                _pending_trusted_word_action_matches(trusted_record, action_name)
            )
        self.assertFalse(
            _pending_trusted_word_action_matches(trusted_record, "换码？")
        )
        self.assertIsNone(_pending_tool_assent_intent(trusted_record, "换码"))
        self.assertIsNone(_pending_tool_assent_intent(trusted_record, "确认"))

        ordinary_state = PendingToolConfirm(
            function_name="keytao_batch_add_to_draft",
            args={"items": []},
        )
        self.assertIsNone(_pending_tool_assent_intent(ordinary_state, "换码"))
        self.assertIsNone(_canonical_draft_management_command("换码"))
...
class ExactMutationBindingTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_dictionary_delete_binds_server_type_and_literal_code_pair(self) -> None:
        item = {
            "action": "Delete",
            "word": "箭头",
            "code": "jmtdu",
        ...
        wrong_type = await self._call(
            "keytao_batch_add_to_draft",
            {"items": [{**item, "type": "Char"}]},
            "把 jmtdu 的箭头删掉",
            **trusted,
        )
        self.assertTrue(wrong_type.get("success"), wrong_type)
        self.assertEqual(self.calls[-1]["items"][0]["type"], "Phrase")

        missing_server_type = await self._call(
            "keytao_batch_add_to_draft",
            {"items": [item]},
            "把 jmtdu 的箭头删掉",
            trusted_word_lookup_codes_by_word={
        ...
        self.assertTrue(
            missing_server_type.get("policyBlocked"),
            missing_server_type,
        )
...
class _PreviousTurnLookupSkills:
    @staticmethod
    def get_skill_instructions():
        return ""
...
class ServerBoundDeleteConfirmationTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_previous_turn_lookup_routes_bare_action_to_one_server_bound_confirmation(self) -> None:
        from keytao_bot.plugins import chat_commands as commands_module
        from keytao_bot.plugins import openai_chat as chat_module

        lookup_reply = "词库中有「箭头」：jmtdu，排序 100。"
        client = _FakeClient([
            _fake_response(
                "tool_calls",
            ...
            _fake_response("stop", lookup_reply),
        ])
        state_store = MemoryConversationStateStore()

        async def lookup(**kwargs):
            self.assertEqual(kwargs["word"], "箭头")
            return {
                "success": True,
                "word": "箭头",
                "phrases": [{
        ...
        orchestrator = AgentOrchestrator(
            ...
            runtime=AgentRuntimeConfig("fake-model", 500, 0.0, 10.0),
            skills_manager=_PreviousTurnLookupSkills(),
            tool_executor=ToolExecutor(
                lambda name: lookup if name == "keytao_lookup_by_word" else None,
                frozenset({"keytao_lookup_by_word"}),
            ),
            state_store=state_store,
            bind_help_text="bind help",
            system_prompt_core="system",
        ...
        context = AgentRequestContext(
            platform="qq",
            user_id="named-delete-owner",
            mutations_allowed=False,
        )

        rendered = await orchestrator.run("查询词库里的箭头", context)
        self.assertTrue(rendered.startswith("「箭头」已在词库（jmtdu）。"))
        self.assertIn("若保留现状，无需操作", rendered)
        self.assertTrue(rendered.endswith(lookup_reply))
        record = state_store.get_record(context.conversation_address)
        self.assertIsNotNone(record)
        self.assertIsInstance(record.state, PendingTrustedWordRecord)
        self.assertEqual(
            (record.state.word, record.state.code, record.state.phrase_type),
            ("箭头", "jmtdu", "Phrase"),
        )
        self.assertTrue(trusted_word_record_is_complete(record.state))
        self.assertFalse(record.requires_reconfirmation)
        ...
        old_delivery_store = chat_module.conversation_state_store
        chat_module.conversation_state_store = state_store
        try:
            self.assertEqual(
            ...
            chat_module.conversation_state_store = old_delivery_store

        preview = {
            "success": False,
            "requiresConfirmation": True,
            "batchId": "named-delete-batch",
            "contentVersion": 4,
            "warningDigest": "a" * 64,
            "warnings": [{
                "warningType": "dictionary_delete",
                "message": "将删除词库中的现有词条",
            }],
        }
        ...
        async def preview_call(tool_name, arguments, *_args, **_kwargs):
            if tool_name == "keytao_get_batch_preview":
                return json.dumps({
                    "success": True,
                    "batchId": "named-delete-batch",
                }, ensure_ascii=False)
            seen_calls.append((tool_name, dict(arguments)))
            self.assertEqual(tool_name, "keytao_batch_add_to_draft")
            self.assertEqual(arguments["items"][0]["word"], "箭头")
            if arguments.get("preview_only"):
                self.assertNotIn("confirmed", arguments)
                return json.dumps(preview, ensure_ascii=False)
            self.assertTrue(arguments.get("confirmed"))
            self.assertEqual(arguments.get("batch_id"), "named-delete-batch")
            self.assertEqual(arguments.get("expected_content_version"), 4)
            self.assertEqual(arguments.get("expected_warning_digest"), "a" * 64)
            return json.dumps({
                "success": True,
                "successCount": 1,
                "failedCount": 0,
                "batchId": "named-delete-batch",
                "draft_snapshot": {"count": 1, "items": [], "summary": {}},
            }, ensure_ascii=False)

        old_state_store = commands_module.conversation_state_store
        commands_module.conversation_state_store = state_store
        try:
            with patch.object(
                commands_module,
                "call_tool_function",
                side_effect=preview_call,
            ) as call:
                ignored = await commands_module.handle_pending_message_core(
                    "确认",
                    "qq",
                    "named-delete-owner",
                    context.conversation_address,
                ...
                self.assertTrue(state_store.set(context.conversation_address, record.state))

                prompt = await commands_module.handle_pending_message_core(
                    "换码",
                    "qq",
                    "named-delete-owner",
                    context.conversation_address,
            ...
            self.assertIn(pending_confirmation_copy(), prompt)
            self.assertIn("箭头", prompt)
            self.assertIn("jmtdu", prompt)
            sealed = state_store.get_record(context.conversation_address)
            self.assertIsInstance(sealed.state, PendingToolConfirm)
            self.assertEqual(
                sealed.state.confirmation_source,
                "server_warning",
            )
            self.assertTrue(sealed.requires_reconfirmation)
            ...
            with patch.object(
                ...
                completed = await commands_module.handle_pending_message_core(
                    "确认",
                    "qq",
                    "named-delete-owner",
                    context.conversation_address,
            ...
            self.assertIn("已加入草稿", completed)
            self.assertIsNone(
                state_store.get_record(context.conversation_address)
            )
            self.assertEqual(len(seen_calls), 2)

            recordless = await commands_module.handle_pending_message_core(
                "换码",
                "qq",
                "named-delete-owner",
                context.conversation_address,
            ...
            self.assertIsNone(recordless)
        finally:
            commands_module.conversation_state_store = old_state_store
...
class CleanBatchAddOrchestratorTests(unittest.IsolatedAsyncioTestCase):
    async def test_existing_multi_word_lookup_leads_with_exact_dictionary_facts(
        self,
    ) -> None:
        """Trusted batch lookup facts lead any model-authored explanation."""
        words = ("参考价值", "情绪价值")
        codes = {"参考价值": "ckjf", "情绪价值": "qxjf"}

        async def dispatch(words=None, **_kwargs):
            self.assertEqual(tuple(words or ()), ("参考价值", "情绪价值"))
            return {
                "success": True,
                "count": 2,
                "results": [
        ...
        client = _FakeClient([
            ...
            _fake_response("stop", "这两个词都可以考虑其他编码。"),
        ])
        orchestrator = AgentOrchestrator(
            client_factory=lambda: client,
            runtime=AgentRuntimeConfig("fake-model", 500, 0.0, 10.0),
        ...
        result = await orchestrator.run(
            ...
            AgentRequestContext(
                platform="qq",
                user_id="existing-multi-owner",
                mutations_allowed=False,
            ),
        )

        self.assertTrue(result.startswith("「参考价值」已在词库（ckjf）。"))
        self.assertIn("「情绪价值」已在词库（qxjf）。", result)
        self.assertIn("若保留现状，无需操作", result)
        self.assertIn("这两个词都可以考虑其他编码", result)
    ...
    async def test_same_chain_batch_candidates_allocate_distinct_slots_by_priority(
        ...
        self.assertNotIn("以上推荐仅用于本次查询", result)
    ...
    async def test_binding_advertisement_uses_only_same_turn_review_records(
        ...
        """A word/code assent offer cannot add a model-only word to its ticket."""
        from keytao_bot.plugins import openai_chat as chat_module

        codes = {"载流": "zhlq", "载流子": "zlzu"}
        model_only_word = "模型虚构词"
        ...
        client = _FakeClient([
            _fake_response(
                ...
                tool_calls=[
                    _named_tool_call(
                        f"call-kind-aware-binding-{index}",
                        "keytao_prepare_reviewed_add",
                        {"word": word},
                    )
        ...
        context = AgentRequestContext(
            platform="qq",
            user_id="binding-kind-owner",
        )

        result = await orchestrator.run("复核载流和载流子", context)
        record = state_store.get_record(context.conversation_address)
        with patch.object(
            chat_module,
            "conversation_state_store",
        ...
        self.assertNotIn("以上推荐仅用于本次查询", delivered)
        self.assertNotIn("加入草稿", delivered)

    async def test_read_only_rereview_advertisement_reestablishes_live_ticket(
        self,
    ) -> None:
        """Every observed layout is backed or replaced from the same records."""
        from keytao_bot.harness import orchestrator as orchestrator_module
        from keytao_bot.plugins import openai_chat as chat_module

        codes = {"显眼包": "xybo", "嘴替": "zbtk"}
        expected_pairs = tuple(codes.items())
        action_copy = (
            "可用的下一步：\n"
            "- 「加入」、「都加」、「添加」→ 只加入草稿\n"
        ...
        layouts = (
            ...
            (
                "live-bracketed-recommendation",
                "已重新复核这两个词，结果如下：\n\n"
                "【显眼包】\n推荐编码：xybo\n候选（全部为空位）：\n"
                "1. xybo — 空位\n2. xyboo — 空位\n\n"
                "【嘴替】\n推荐编码：zbtk\n候选（全部为空位）：\n"
                "1. zbtk — 空位\n2. zbtko — 空位\n\n"
                "可继续之前确认的写入计划：显眼包 → xybo，嘴替 → zbtk。"
                "你可直接回复「加入并提交」完成加入并提交。",
                True,
            ),
            (
                "garbled-unparseable",
        ...
        for case_index, (label, model_reply, _expects_replacement) in enumerate(layouts):
            with self.subTest(layout=label):
                ...
                async def dispatch(word=None, items=None, **_kwargs):
                    ...
                    alternate = codes[word] + "o"
                    reviewed["candidateCodes"] = [codes[word], alternate]
                    reviewed["candidateStatuses"] = [
                        {"code": codes[word], "occupied": False},
                        {"code": alternate, "occupied": False},
                    ]
                    return reviewed
                ...
                client = _FakeClient([
                    _fake_response(
                        ...
                        tool_calls=[
                            _named_tool_call(
                                f"call-rereview-{case_index}-{index}",
                                "keytao_prepare_reviewed_add",
                                {"word": word},
                            )
                            for index, word in enumerate(codes)
                ...
                context = AgentRequestContext(
                    platform="qq",
                    user_id=f"rereview-owner-{case_index}",
                    space_type="group",
                    space_id="865189947",
                    speaker_name="Rea",
                    mutations_allowed=False,
    ...
    async def test_scan_advertisement_uses_only_same_turn_absent_lookup_records(
        ...
        """A no-code inventory offer is backed by the exact absent lookup set."""
        from keytao_bot.plugins import openai_chat as chat_module
        ...
        model_reply = (
            "未收录词：\n"
            + "\n".join(f"- {word}" for word in (*words, model_only_word))
            + "\n可以把这 4 个词加入草稿。"
        )
        client = _FakeClient([
            _fake_response(
        ...
        context = AgentRequestContext(
            platform="qq",
            user_id="scan-kind-owner",
            space_type="group",
            space_id="scan-kind-group",
            speaker_name="Owner",
        )

        result = await orchestrator.run("扫描这些常用词", context)
        record = state_store.get_record(context.conversation_address)
        with patch.object(
            chat_module,
            "conversation_state_store",
        ...
        self.assertIsNotNone(record)
        self.assertIsInstance(record.state, PendingAdvertisedWordSets)
        self.assertEqual(len(record.state.snapshots), 1)
        self.assertEqual(record.state.snapshots[0].words, words)
        self.assertEqual(delivered, result)
        self.assertTrue(all(word in delivered for word in words))
        self.assertNotIn(model_only_word, delivered)

    async def test_mixed_advertisement_prefers_specific_review_bindings(
        self,
    ) -> None:
        """A mixed reply is normalized to one candidate ticket, never two authorities."""
        from keytao_bot.plugins import openai_chat as chat_module

        candidate_codes = {"载流": "zhlq", "载流子": "zlzu"}
        absent_words = ("显眼包", "嘴替")
        model_only_word = "模型虚构词"

        async def dispatch(word=None, words=None, **_kwargs):
            if words is not None:
                return {
                    "success": True,
                    "count": len(words),
        ...
        mixed_reply = (
            ...
            + pending_batch_confirmation_copy()
            + "\n另一个未收录列表："
            + "、".join((*absent_words, model_only_word))
            + "。可以把列表中的词加入草稿。"
        )
        client = _FakeClient([
            _fake_response(
                ...
                tool_calls=[
                    _named_tool_call(
                        "call-kind-aware-mixed-lookup",
                        "keytao_lookup_by_words_batch",
                        {"words": list(absent_words)},
                    ),
                    *[
                        _named_tool_call(
                            f"call-kind-aware-mixed-review-{index}",
                            "keytao_prepare_reviewed_add",
                            {"word": word},
                        )
                        for index, word in enumerate(candidate_codes)
                    ],
                ],
            ),
            _fake_response("stop", mixed_reply),
        ])
        state_store = MemoryConversationStateStore()
        orchestrator = AgentOrchestrator(
            client_factory=lambda: client,
        ...
        context = AgentRequestContext(
            platform="qq",
            user_id="mixed-kind-owner",
        )

        result = await orchestrator.run("扫描并复核", context)
        record = state_store.get_record(context.conversation_address)
        with patch.object(
            chat_module,
            "conversation_state_store",
    ...
    async def test_advertisement_without_either_record_kind_has_no_command(
        ...
        """Model prose alone creates no ticket or word-set snapshot."""
        from keytao_bot.plugins import openai_chat as chat_module

        model_only_word = "模型虚构词"
        model_reply = (
            f"未收录词：{model_only_word}。"
            "可以把列表中的词加入草稿。"
        )
        ...
        context = AgentRequestContext(
            platform="qq",
            user_id="missing-kind-owner",
        )

        result = await orchestrator.run("扫描", context)
        with patch.object(
            chat_module,
            "conversation_state_store",
            state_store,
    ...
    async def test_advertised_candidate_reply_cannot_mint_unreviewed_code(self) -> None:
        ...
        self.assertNotEqual(result, forged_reply)
...
class PendingPositionalCreateOrchestratorTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_partial_code_lookup_cannot_hide_destination_ambiguity(self) -> None:
        ...
        self.assertIn("若保留现状，无需操作", result)
        self.assertTrue(result.endswith("destination is ambiguous"))
...
class FinalReplyLoopBreakerTests(unittest.TestCase):
    ...
    def test_user_facing_guard_rejects_tool_ids_and_model_directives_in_any_spelling(self) -> None:
        from keytao_bot.plugins.chat_render import _assert_plain_user_facing_reply

        for fragment in (
            "工具 keytao_shift_phrase_code 首次调用失败",
            "工具 keytaoshiftphrase_code 首次调用失败",
            "工具 keytao-shift-phrase-code 首次调用失败",
            "禁止重复调用。",
            "请直接根据上方结果回复用户。",
    ...
    def test_internal_duplicate_hint_is_replaced_but_partial_receipt_is_reported(self) -> None:
        receipt = AgentOrchestrator._successful_write_receipt(
            "keytao_shift_phrase_code",
            {"word": "剪贴", "target_code": "jmtdoa"},
            {
                "success": True,
                "batchId": "partial-shift",
                "shiftPlan": {
                    "shifted": [],
                    "currentState": [{
                        "word": "剪贴", "code": "jmtd",
        ...
        finalized = AgentOrchestrator._finalize_reply(
            "把「箭头」换到jmtd，把「剪贴」换到jmtdoa",
            (
                "后续操作未完成；原因：工具 keytaoshiftphrase_code 首次调用"
                "（已被安全层拦截或执行未成功）。禁止重复调用。"
                "请直接根据上方结果回复用户。"
            ),
            {
                "success": False,
                "message": (
                    "工具 keytaoshiftphrase_code 首次调用未成功。"
                    "禁止重复调用。请直接根据上方结果回复用户。"
                ),
                "_failedTool": "keytao_shift_phrase_code",
            },
            [receipt],
        )

        self.assertIn("剪贴", finalized)
        self.assertIn("jmtd → jmtdoa", finalized)
        self.assertIn("后续操作未完成", finalized)
        self.assertNotRegex(
            finalized,
            r"keytao|禁止(?:再次|重复)?调用|请直接根据.*回复用户",
        )
    ...
    def test_live_candidate_binding_failure_never_demands_full_operands(self) -> None:
        ...
        executor = ToolExecutor(
            lambda name: execute if name == "keytao_create_phrase" else None,
            frozenset({"keytao_create_phrase"}),
        )
        blocked = json.loads(asyncio.run(executor.call(
            "keytao_create_phrase",
            {"word": "达致", "code": "dsfkv"},
            ToolContext(
                platform="qq",
                user_id="3755240737",
                current_message="加入草稿，然后就提交。",
                writes_allowed=True,
                pending_candidate=PendingCandidateCapability(
                    state_matches=True,
                    word="达致",
                    candidates=(("dsfk", True), ("dsfkv", False)),
                    occupied_words=(("dsfk", ("大致",)),),
        ...
        self.assertEqual(calls, [])
        self.assertEqual(blocked.get("blockReason"), "binding_incomplete")
        self.assertTrue(blocked.get("liveCandidateSelected"))
        self.assertEqual(blocked.get("suggestedCommand"), "加入并提交")
        self.assertNotIn("达致", blocked.get("suggestedCommand", ""))
        self.assertNotIn("dsfkv", blocked.get("suggestedCommand", ""))

        finalized = AgentOrchestrator._finalize_reply(
            "加入草稿，然后就提交。",
            "请复制发送下面这行完整指令：添加 达致 dsfkv 并提交",
            blocked,
        )
        self.assertIn("当前候选", finalized)
        self.assertIn("- 「加入并提交」", finalized)
        self.assertNotIn("完整指令", finalized)
        self.assertNotIn("添加 达致 dsfkv", finalized)
