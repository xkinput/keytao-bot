        ...
        async def run():
            with tempfile.TemporaryDirectory() as directory:
                ...
                with patch.object(chat, "conversation_state_store", reloaded), patch.object(
                    ...
                    self.assertIn("常用度", receipt)
                    self.assertIn("未加入", receipt)
                    receipt = await commands.handle_pending_message_core("小端 3", "qq", "s54-offline", key, allow_intent_model=False)
                    execute.assert_awaited_once()
                selected = execute.await_args.args[0]
                self.assertEqual([(item["word"], item["code"]) for item in selected.args["items"]], [("小端", "xcdtvo")])
